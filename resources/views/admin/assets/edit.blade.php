<x-admin-layout title="Edit Aset">
                ...
                <form method="POST" action="{{ route('admin.assets.update', $asset->id) }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <select id="category" name="category" required
                                ...
                                @php
                                $categories = ['Computer', 'Peripheral', 'Networking', 'Storage', 'Software',
                                'Furniture', 'Other'];
                                @endphp
                                @foreach ($categories as $category)
                                <option value="{{ $category }}" {{ old('category', $asset->category ?? '') === $category
                                    ? 'selected' : '' }}>
                                    {{ $category }}
                                </option>
                                @endforeach
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea id="description" name="description" rows="5"
                            ...
                            class="w-full px-4 py-4 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 bg-gray-50 text-gray-900 placeholder:text-gray-400 resize-vertical">{{ old('description', $asset->description) }}</textarea>
                        @error('description')
                        <div class="flex items-center mt-2 text-red-600 text-sm">
                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <select id="status" name="status" required
                                class="w-full px-4 py-4 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 bg-gray-50 text-gray-900">
                                <option value="Available" @if(old('status', $asset->status) == 'Available') selected
                                    @endif>Tersedia</option>
                                <option value="Damaged" @if(old('status', $asset->status) == 'Damaged') selected
                                    @endif>Rusak</option>
                                <option value="Borrowed" @if(old('status', $asset->status) == 'Borrowed') selected
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" id="purchase_price" name="purchase_price" placeholder="5000000"
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-center p-4 bg-gray-50 border border-gray-200 rounded-xl mb-4">
                            <img src="{{ asset('storage/' . $asset->asset_image_path) }}" alt="Foto saat ini"
